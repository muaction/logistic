...
require TID_PATH . 'inc/import/parsers.php';
...
if ( class_exists( 'WP_Importer' ) ) {
	...
	class WP_Import extends WP_Importer {
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( !empty( $this->authors ) ) : ?>
					<h3><?php esc_attr_e( 'Assign Authors', 'tid' ); ?></h3>
					<p><?php esc_attr_e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'tid' ); ?></p>
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					<h3><?php esc_attr_e( 'Import Attachments', 'tid' ); ?></h3>
					<p>
						<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
						<label for="import-attachments"><?php esc_attr_e( 'Download and import file attachments', 'tid' ); ?></label>
					</p>
				<?php endif; ?>

				<p class="submit">
					<input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'tid' ); ?>" /></p>
			</form>
			<?php
		}
		...
		function author_select( $n, $author ) {
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					esc_attr_e( 'or create new user with login name:', 'tid' );
					$value = '';
				} else {
					esc_attr_e( 'as a new user:', 'tid' );
			...
			if ( !$create_users && $this->version == '1.0' ) {
				esc_attr_e( 'assign posts to an existing user:', 'tid' );
			} else {
				esc_attr_e( 'or assign posts to an existing user:', 'tid' );
			}
			wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', 'tid' ) ) );
		...
		function process_attachment( $post, $url ) {
			if ( !$this->fetch_attachments ) {
				return new WP_Error( 'attachment_processing_error', __( 'Fetching attachments is not enabled', 'tid' ) );
			}
			...
			if ( !$this->method ) {
				...
				if ( !$attachment_exist ) {
					...
					if ( $info = wp_check_filetype( $upload['file'] ) ) {
						...
						return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'tid' ) );
					}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( !$headers ) {
				...
				return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'tid' ) );
			}
			...
			if ( $headers['response'] != '200' ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote server returned error response %1$d %2$s', 'tid' ), esc_html( $headers['response'] ), get_status_header_desc( $headers['response'] ) ) );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				...
				return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'tid' ) );
			}

			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );

				return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'tid' ) );
			}
			...
			if ( !empty( $max_size ) && $filesize > $max_size ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'tid' ), size_format( $max_size ) ) );
			}
...
//			}
		...
		function header() {
			echo '<h2>' . __( 'Import WordPress', 'tid' ) . '</h2>';
...
if ( !function_exists( 'wordpress_importer_init' ) ) {
	...
	function wordpress_importer_init() {
		...
		register_importer( 'wordpress', 'WordPress', __( 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', 'tid' ), array( $GLOBALS['wp_import'], 'dispatch' ) );
	}
