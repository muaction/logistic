	...
	class THIM_Our_Team {
	    ...
	    public function template_redirect() {
	    	...
	    	if (get_post_type() == "our_team" && (is_category() || is_archive()) ) {
	    		$this->get_template_part( 'archive' , "our-team");
	    		exit();
	    	}
	    	else if (get_post_type() == "our_team" && is_single()) {
	    		$this->get_template_part("single", "our-team");
	    		exit();
	    	}
	    	else {
	    		return;
	    ...
	    public function register_cptu () {
	    	$labels = array(
				'name'               => _x( 'Our Team', 'Post Type General Name', 'thimpress' ),
				'singular_name'      => _x( 'Our Team', 'Post Type Singular Name', 'thimpress' ),
				'menu_name'          => __( 'Our Team', 'thimpress' ),
				'parent_item_colon'  => __( 'Parent Our Team:', 'thimpress' ),
				'all_items'          => __( 'All Our Teams', 'thimpress' ),
				'view_item'          => __( 'View Our Team', 'thimpress' ),
				'add_new_item'       => __( 'Add New Our Team', 'thimpress' ),
				'add_new'            => __( 'New Our Team', 'thimpress' ),
				'edit_item'          => __( 'Edit Our Team', 'thimpress' ),
				'update_item'        => __( 'Update Our Team', 'thimpress' ),
				'search_items'       => __( 'Search our team', 'thimpress' ),
				'not_found'          => __( 'No our team found', 'thimpress' ),
				'not_found_in_trash' => __( 'No our team found in Trash', 'thimpress' ),
			);
			$args   = array(
				'labels'      => $labels,
		...
		public function register_taxonomy () {
			// Our_Team Categories
			$labels = array(
				'name'                       => _x( 'Our Team Categories', 'Taxonomy General Name', 'thimpress' ),
		...
		public function updated_messages( $messages = array() ) {
			...
			$messages['our_team'] = array(
				0  => '',
				1  => sprintf( __( 'Our Team updated. <a href="%s">View Our Team</a>', 'thimpress' ), esc_url( get_permalink( $post_ID ) ) ),
				2  => __( 'Custom field updated.', 'thimpress' ),
				3  => __( 'Custom field deleted.', 'thimpress' ),
				4  => __( 'Our Team updated.', 'thimpress' ),
				5  => isset( $_GET['revision'] ) ? sprintf( __( 'Our Team restored to revision from %s', 'thimpress' ), wp_post_revision_title( ( int ) $_GET['revision'], false ) ) : false,
				6  => sprintf( __( 'Our Team published. <a href="%s">View Our Team</a>', 'thimpress' ), esc_url( get_permalink( $post_ID ) ) ),
				7  => __( 'Our Team saved.', 'thimpress' ),
				8  => sprintf( __( 'Our Team submitted. <a target="_blank" href="%s">Preview Our Team</a>', 'thimpress' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
				9  => sprintf( __( 'Our Team scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Our Team</a>', 'thimpress' ), date_i18n( __( 'M j, Y @ G:i', 'thimpress' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ) ),
				10 => sprintf( __( 'Our Team draft updated. <a target="_blank" href="%s">Preview Our Team</a>', 'thimpress' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
			);
		...
		public function our_team_register_metabox ($meta_boxes) {
			$meta_boxes[] = array(
		        ...
		        'fields' => array(
					...
					array(
						'name' => __( 'Dribbble URL', 'agapi' ),
						'id'   => 'dribbble_url',
						'type' => 'textfield',
						'desc'=>''
					),
					array(
						'name' => __( 'Linked In URL', 'agapi' ),
						'id'   => 'linkedin_url',
						'type' => 'textfield',
						'desc'=>''
					),
